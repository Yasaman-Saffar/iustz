...
public:
    Michelangelo(string name , string gender , int age) : Player(name , gender , age , 0 , 100 , 50 , 20){}//base of level,hp , stamina, money
...
class Shop{
	friend class Player;
private:
    vector<Weapon> weapons;// name , cost , damage
...
int main()
{
	...
	ShowMenue(player);
	Michelangelo("mm" , ",mn" , 21);
	Shop shop;
	shop.ShowOfShop(player);
	BackPack backpack;
	shop.BuyWeapon(player , backpack);
    return 0;
}