...
class characters
{
    ...
    characters(string name, int age, string gender)
    :Name(name), Age(age), Gender(gender), Level(1), Stamina(50), HP(100), Money(100){}
...
class Michelangelo : public characters
{
    ...
    Michelangelo(string name, int age, string gender)
...
characters* PreChar(characters* Player)
{
    ...
    while(getline(PreChar, line))
    {
        istringstream iss(line);
        if(num == 1)
        {
            if(line == "Michelangelo")
                Player = new Michelangelo("y", 0, "", 0, 0, 0, 0);
...
void ChoosingChar()
{
    ...
    while(!isValid)
    {
        ...
        if(choice1 == 1)
        {
            ...
            if(choice2 == 3)
                player = new Michelangelo(name, age, gender);

            isValid = true;
        }
        if(choice1 == 2)
        {
            player = PreChar(player);
            isValid = true;
        }
